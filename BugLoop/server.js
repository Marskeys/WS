...
// 🔥 [이동됨] sitemapRoutes와 sitemapPagesRoutes는 모든 Lang/Root 라우트 뒤에 배치됩니다.
...
// -------------------------------------------------------
// 2-3. Lang이 누락된 요청 처리 (301 영구 보정 - 무한 루프 방지)
// -------------------------------------------------------

// 🚨 Lang 누락 시 /ko/... 로 301 영구 이동
...
// ✅ 루트 접근 시 /ko/ 로 이동 (302 임시 이동)
app.get('/', (req, res) => res.redirect(302, '/ko/'));

// ✅ 언어 코드만 있는 요청 (/ko, /en)은 /ko/ 로 슬래시 보정 (302 임시 이동)
...
// =======================================================
// ✅ [수정] 3. Sitemap 라우트 (라우팅 체인 가장 마지막에 배치)
// =======================================================
// 🔥 Sitemap 라우트는 모든 Lang/Root 라우트 처리 이후에 실행되어야 충돌을 피하고, 
//    자체적인 리다이렉트 루프를 방지합니다.
app.use('/', sitemapRoutes);
app.use('/', sitemapPagesRoutes);
