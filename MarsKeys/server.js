  ...
  try {
    ...
    posts.forEach(post => {
      supportedLangs.forEach(lang => {
        postUrls.push(`
          <url>
            <loc>https://marskeys.com/${lang}/post/${post.id}</loc>
    ...
    // 정적 페이지 URL을 생성합니다. 도메인은 marskeys.com로 설정되어 있습니다.
    const staticUrls = [
      ...supportedLangs.map(lang => `<url><loc>https://marskeys.com/${lang}/</loc><priority>1.00</priority></url>`),
      ...supportedLangs.map(lang => `<url><loc>https://marskeys.com/${lang}/signup</loc><priority>0.80</priority></url>`)
    ].join('');
...
app.get('/post/:id', async (req, res) => {
  try {
    ...
    // 3. 게시글의 원본 카테고리를 파싱하고, 각 카테고리의 번역된 이름을 조회
